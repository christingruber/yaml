...
class LintCommand extends Command
{
    ...
    public function __construct(string $name = null, callable $directoryIteratorProvider = null, callable $isReadableProvider = null, string $linterConfig = 'yamllinter.xml')
    {
...
You can validate YAML contents passed from config file:
  ...
  <info>php %command.full_name% yamllinter.xml</info>
...
EOF
    ...
    private function parseLinterConfig(string $config): array
    {
        ...
        if (isset($xml->{'format'})) {
            $result['format'] = $xml->{'format'};
        }

        if (isset($xml->{'parse-tags'})) {
            $result['parse-tags'] = $xml->{'parse-tags'};
        }

        if ($xml->xpath('//includes/path')) {
            $result['includes'] = $xml->xpath('//includes/path')[0];
        }

        if ($xml->xpath('//excludes/path')) {
            $result['excludes'] = $xml->xpath('//excludes/path')[0];
        }
